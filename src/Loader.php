...
class Loader {
   ...
   * @return array
   */
  public static function mapSubclassesAvailable($folder_name, $parent_class_name, $include = FALSE) {
   ...
   * @return array
   */
  public static function mapImplementationsAvailable($folder_name, $interface_name) {
    self::includeLibClassFilesWithPattern($folder_name);
   ...
   */
  public static function getSubclassImplementations($parent_class_name) {
    $classes = array();
    foreach (get_declared_classes() as $class_name) {
      $reflection_class = new \ReflectionClass($class_name);
      if ($reflection_class->isSubclassOf($parent_class_name)) {
        $classes[self::getComponentType($class_name)] = $class_name;
      }
    }
    return $classes;
  }
  ...
  private static function getComponentType($class_name) {
   ...
   * @return type
   */
  private static function listFoldersWithPattern($dir, $pattern) {
    $folders = array();
    $ffs = scandir($dir);
    foreach ($ffs as $ff) {
      if ($ff != '.' && $ff != '..') {
   ...
   * @return array
   */
  private static function listClassesWithinFolder($dir) {
    $classes = array();
    $ffs = scandir($dir);
    foreach ($ffs as $ff) {
      if ($ff != '.' && $ff != '..') {
